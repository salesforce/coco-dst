...
class TRADE(nn.Module):
    ...
    def save_model(self, dec_type):
        directory = args['out_dir']+'/TRADE-'+args["addName"]+args['dataset']+str(self.task)+'/'+'HDD'+str(self.hidden_size)+'BSZ'+str(args['batch'])+'DR'+str(self.dropout)+str(dec_type)                 
    ...
    def query(self, dev, matric_best, slot_temp, ignore_none_dontcare):
        # Set to not-training mode to disable dropout
        self.encoder.train(False)
        self.decoder.train(False)
        all_prediction = {}
        ...
        if(ignore_none_dontcare):
            pred_belief, target_belief = self.ignore_none_dontcare(predict_belief_bsz_ptr,data_dev["turn_belief"][bi])
        else:
            pred_belief , target_belief = predict_belief_bsz_ptr,data_dev["turn_belief"][bi]

        if(set(pred_belief) == set(target_belief)):
            joint_acc = 1.0
        else:
            joint_acc = 0.0
        return{"Joint Acc": joint_acc ,"Ground Truth" :target_belief, "Prediction": pred_belief}
         

    def ignore_none_dontcare(self,pred_belief, target_belief):

        clean_target_belief = []
        clean_pred_belief = []
        for bs in target_belief:
            if 'not mentioned' in bs or 'dontcare' in bs or "none" in bs:
                continue
    ...
    def evaluate_metrics(self, all_prediction, from_which, slot_temp):
        ...
        for d, v in all_prediction.items():
            for t in range(len(v)):
                cv = v[t]
                pred_belief, target_belief = self.ignore_none_dontcare(cv[from_which],cv["turn_belief"])
                cv["turn_belief"] = target_belief
                cv[from_which] = pred_belief
...
class EncoderRNN(nn.Module):
    def __init__(self, vocab_size, hidden_size, dropout, n_layers=1):
        ...
        if args["load_embedding"]:
            with open(os.path.join(args['out_dir'], 'emb{}.json'.format(vocab_size))) as f:
                E = json.load(f)
