...
def get_train_data_point(gating_dict,dial_dict,turnID,user_input,belief_state,slot_temp,max_value_len,max_resp_len,mem_lang,do_normalize):
    ...
    for ti, turn in enumerate(dial_dict["dialogue"]):
        if(ti < turnID):
            ...
            turn_uttr = system_transcript+ " ; " + transcript
            turn_uttr_strip = turn_uttr.strip()
            dialog_history +=  (system_transcript + " ; " + transcript + " ; ")
            source_text = dialog_history.strip()
        else:
            turn_domain = turn["domain"]
            turn_id = turn["turn_idx"]
            system_transcript = turn["system_transcript"]
            if(do_normalize):
                transcript = normalize(user_input,False)
            else:
    ...
    def norm_belief_state(turn_belief):
        for idx,state in enumerate(turn_belief):
            slot,value = state["slots"][0]
            value = normalize(value,False)
            state["slots"][0][1] = value
            turn_belief[idx] = state
            
        return turn_belief
    if(do_normalize):
        belief_state = norm_belief_state(belief_state)
    turn_belief_dict = fix_general_label_error(belief_state, False, slot_temp)
    # Generate domain-dependent slot list
    turn_belief_list = [str(k)+'-'+str(v) for k, v in turn_belief_dict.items()]
    mem_lang.index_words(turn_belief_dict, 'belief')
    class_label, generate_y, slot_mask, gating_label  = [], [], [], []
    ...
    if(do_normalize):
        data_ID = dial_dict["dialogue_idx"] + "_aug"
    else:
        data_ID = dial_dict["dialogue_idx"]
    ...
    return data_detail, max_value_len , max_resp_len



def read_aug_langs(file_name, gating_dict, SLOTS, dataset, lang, mem_lang, sequicity, training):
    print(("Reading from {}".format(file_name)))
    data = []
    max_resp_len, max_value_len = 0, 0
    print("read aug_file from: ",args["aug_file"])
    with open(args["aug_file"]) as f:
    ...
    with open(file_name) as f:
            ...
            if (args["all_vocab"] or dataset=="train") and training:
                for ti, turn in enumerate(dial_dict["dialogue"]):
                    ...
                    if(key in aug_dials and aug_dials[key]["success"]):
                        aug_transcript = normalize(aug_dials[key]["new_utter"],False)
                        lang.index_words(aug_transcript, 'utter')
        # determine training data ratio, default is 100%
        if training and dataset=="train" and args["data_ratio"]!=100:
            random.Random(10).shuffle(dials)
            dials = dials[:int(len(dials)*0.01*args["data_ratio"])]
        ...
        for dial_dict in dials:
            for ti, turn in enumerate(dial_dict["dialogue"]):
                data_detail, max_value_len , max_resp_len = get_train_data_point(gating_dict,dial_dict,ti,turn["transcript"],turn["belief_state"],SLOTS,
                                                                            max_value_len,max_resp_len,mem_lang,do_normalize = False)
                data.append(data_detail)
                key = (dial_dict["dialogue_idx"]+str(ti))
                if(key in aug_dials and aug_dials[key]["success"]):
                    data_detail, max_value_len , max_resp_len = get_train_data_point(gating_dict,dial_dict,ti,aug_dials[key]["new_utter"],
                                                aug_dials[key]["belief_state"],SLOTS, max_value_len,max_resp_len,mem_lang, do_normalize = True)
                    # import pdb;
                    # pdb.set_trace()
                data.append(data_detail)
    ...
    return data, max_resp_len, SLOTS
...
def dump_pretrained_emb(word2index, index2word, dump_path):
    ...
    print("Finish loading pretrained embeddings")
    E = []
    ...
    print("Dumped pretrained embeddings")
...
def prepare_data_seq(training, task="dst", sequicity=0, batch_size=100):
    ...
    file_train = '../multiwoz/MultiWOZ_2.1/trade_train_dials.json'
    file_dev = '../multiwoz/MultiWOZ_2.1/trade_dev_dials.json'
    file_test = '../multiwoz/MultiWOZ_2.1/trade_test_dials.json'
    # Create saving folder
    if args['path']:
        folder_name = args['path'].rsplit('/', 2)[0] + '/'
    else:
        folder_name = args['out_dir']+'/{}-'.format(args["decoder"])+args["addName"]+args['dataset']+str(args['task'])+'/'
    print("folder_name", folder_name)
    if not os.path.exists(folder_name): 
        os.makedirs(folder_name)
    # load domain-slot pairs from ontology
    ontology = json.load(open("../multiwoz/MultiWOZ_2.1/trade_ontology.json", 'r'))
    ...
    if training:
        ...
        if(args["aug_file"]):
            pair_train, train_max_len, slot_train = read_aug_langs(file_train, gating_dict, ALL_SLOTS, "train", lang, mem_lang, sequicity, training)
        else:
        ...
        emb_dump_path = args['out_dir']+'/emb{}.json'.format(len(lang.index2word))
...
def read_GUI_langs(dial_dict,turnID,user_input, gating_dict, SLOTS, dataset, lang, mem_lang, sequicity, training, max_line = None):
    ...
    for ti, turn in enumerate(dial_dict["dialogue"]):
        if(ti < turnID):
            turn_domain = turn["domain"]
            turn_id = turn["turn_idx"]
            turn_uttr = normalize(turn["system_transcript"],False)+ " ; " + normalize(turn["transcript"],False)
            turn_uttr_strip = turn_uttr.strip()
            dialog_history +=  (normalize(turn["system_transcript"],False) + " ; " + normalize(turn["transcript"],False) + " ; ")
            source_text = dialog_history.strip()
            continue
        else:
            turn_domain = turn["domain"]
            turn_id = turn["turn_idx"]
            turn_uttr = normalize(turn["system_transcript"],False) + " ; " + normalize(user_input,False)
            turn_uttr_strip = turn_uttr.strip()
            dialog_history +=  (normalize(turn["system_transcript"],False) + " ; " + normalize(user_input,False) + " ; ")
            source_text = dialog_history.strip()
            break
    ...
    turn["belief_state"] = norm_belief_state(turn["belief_state"])
    turn_belief_dict = fix_general_label_error(turn["belief_state"], False, SLOTS)
...
def prepare_GUI_data_seq(dial,turnID,user_input,training, task="dst", sequicity=0, batch_size=1):
    ...
    pair_train, train_max_len, slot_train, train, nb_train_vocab = [], 0, {}, [], 0
    pair_dev, dev_max_len, slot_dev, dev, nb_dev_vocab = [], 0, {}, [], 0
    pair_test, test_max_len, slot_test = read_GUI_langs(dial,turnID,user_input,gating_dict, ALL_SLOTS, "test", lang, mem_lang, sequicity, training)
    test  = get_seq(pair_test, lang, mem_lang, eval_batch, False, sequicity)    
    max_word = 660
    SLOTS_LIST = [ALL_SLOTS, slot_train, slot_dev, slot_test]
    # print("[Test Set Slots]: Number is {} in total".format(str(len(SLOTS_LIST[3]))))
